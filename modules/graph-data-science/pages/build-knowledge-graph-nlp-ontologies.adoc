...
:tags: nlp, knowledge-graph, neosemantics, apoc
:description: This guide shows how to build and query a Knowledge Graph of entities extracted using APOC NLP procedures and Ontologies extracted using neosemantics.
:page-comments:
:page-type: Tutorial
...
Having Neo4j Desktop link:/download[downloaded^] and installed will allow you to code along with the examples.
...
image:{img}/apoc-small.jpg[float="right", width="100px"]

https://neo4j.com/labs/apoc/[APOC^] (Awesome Procedures on Neo4j) is Neo4j's standard library.
It contains more than 450 procedures and functions providing functionality for utilities, conversions, graph updates, and more.
We're going to use this tool to scrape web pages and apply NLP techniques on text data.

We can install APOC from the plugins section of a database in the Neo4j Desktop:

.Installing APOC
image::{img}/apoc-plugin-install.png[Installing APOC, role="popup-link"]

We'll also need to install the APOC NLP Dependencies jar from GitHub releases.
At the time of writing, the latest version of APOC is 4.0.0.18, so we need to download `apoc-nlp-dependencies-4.0.0.18.jar` from the https://github.com/neo4j-contrib/neo4j-apoc-procedures/releases/tag/4.0.0.18[4.0.0.18 release page^].
Once we've downloaded that file, we need to place it in the `plugins` directory:

.Installing APOC NLP Dependencies
image::{img}/apoc-dependencies.png[Installing APOC NLP Depedencies, role="popup-link"]

[#neosemantics]
=== neosemantics (n10s)

image:{img}/n10s.png[float="right", width="100px"]
...
image:{img}/1280px-Wikidata-logo-en.svg.png[float="right", width="100px"]
...
image::{img}/wikidata-api.png[Querying a Software Taxonomy in the Wikidata SPARQL API]
...
[source,cypher]
----
include::example$nlp/queries.cypher[tag="constraint"]
----

[source,cypher]
----
include::example$nlp/queries.cypher[tag="init"]
----

[source,cypher]
----
include::example$nlp/queries.cypher[tag="mappings"]
----
...
image::{img}/copy-sparql-api.png[Getting an importable URI for a Wikidata SPARQL query, role="popup-link"]
...
[source,cypher]
----
include::example$nlp/queries.cypher[tag="software-systems"]
----
...
[source,cypher]
----
include::example$nlp/queries.cypher[tag="programming-languages"]
----
...
[source,cypher]
----
include::example$nlp/queries.cypher[tag="data-formats"]
----
...
[source,cypher]
----
include::example$nlp/queries.cypher[tag="version-control"]
----

.Version Control Graph
image::{img}/version-control.svg[Version Control Graph, role="popup-link"]
...
image::{img}/devto-screenshot.png[The dev.to home page, role="popup-link"]
...
[source,cypher]
----
include::example$nlp/queries.cypher[tag="dev-to-articles"]
----
...
We'll use the https://neo4j.com/labs/apoc/4.1/overview/apoc.periodic/apoc.periodic.iterate/[`apoc.periodic.iterate`^] procedure so that we can parallelise this process.
...
[source,cypher]
----
include::example$nlp/queries.cypher[tag="dev-to-import"]
----
...
image::{img}/post-dev-to-import.png[Articles and Taxonomies Sub Graphs, role="popup-link"]
...
image:{img}/apoc_gcp.jpg[float="right", width="100px"]
...
[source,cypher]
----
include::example$nlp/queries.cypher[tag="set-key"]
----
...
image::{img}/one-article-entities.svg[One article's entity graph, role="popup-link"]
...
[source,cypher]
----
include::example$nlp/queries.cypher[tag="nlp-import"]
----
...
[source,cypher]
----
include::example$nlp/queries.cypher[tag="semantic-search"]
----
...
[source,cypher]
----
include::example$nlp/queries.cypher[tag="zookeeper"]
----

.Zookeeper to NoSQL
image::{img}/zookeeper-nosql.png[Zookeeper to NoSQL,role="popup-link"]
...
[source,cypher]
----
include::example$nlp/queries.cypher[tag="similar-articles-1"]
----

.Finding similar articles
image::{img}/similar-articles-1.png[Finding similar articles, role="popup-link"]
...
[source,cypher]
----
include::example$nlp/queries.cypher[tag="similar-articles-2"]
----
...
[source,cypher]
----
include::example$nlp/queries.cypher[tag="custom-ontology-init"]
----
...
[source,cypher]
----
include::example$nlp/queries.cypher[tag="custom-ontology-preview"]
----

.Software Stacks Ontology
image::{img}/stacks.svg[Software Stacks Ontology, role="popup-link"]

It looks good, so let's import it by running the following query:

[source,cypher]
----
include::example$nlp/queries.cypher[tag="custom-ontology-import"]
----
