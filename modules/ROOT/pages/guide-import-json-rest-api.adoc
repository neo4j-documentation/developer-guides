...
All that's left to do now is wrap the whole thing in periodic commit. We'll call `apoc.periodic.commit` method with two arguments: the first is the Cypher commands (as a string) to run until the `RETURN` clause returns 0, and the second is a map of `{ propertyName: value }` parameters which will be available (with the dollar sign in front, like `$propertyName`) when the Cypher commands are evaluated.  
