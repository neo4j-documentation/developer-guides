...
NOTE: There is now an official https://neo4j.com/docs/browser-manual/current/[Neo4j Browser Manual^] and also the option to https://neo4j-browser.canny.io/feature-requests[send feature requests^] and https://neo4j-browser.canny.io/changelog[read the change-log^].
