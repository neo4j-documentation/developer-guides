...
1. Please stop your Neo4j server (only for Community Edition, which doesn't have the "multi-database" feature).
2. Then you can import the file using the `./bin/neo4j-admin load  --force true --from file.dump --database <dbname>` command.
3. Make the new database known to the system database with `CREATE DATABASE dbname` which will also automatically start it
