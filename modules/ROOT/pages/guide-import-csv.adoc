...
* link:https://neo4j.com/graphacademy/online-training/introduction-to-neo4j-40/[Introduction to Neo4j 4.0: Using LOAD CSV for Import]
...
[source]
----
customerId:ID(customer-reference), name
23, Delicatessen Inc
42, Delicious Bakery
----
...
[source]
----
productId:ID(product-reference), name, price, :LABEL
...
[source]
----
orderId:ID(order-reference),date,total,customerId:IGNORE
----

.customer_orders_header.csv
[source]
----
:END_ID(order-reference),date:IGNORE,total:IGNORE,:START_ID(customer-reference)
----
...
[source]
----
:START_ID(order-reference),amount,price,:END_ID(product-reference)
1041,13,130,11
1042,2,20,11
----
...
** if you have repeated IDs across entities, you have to provide the entity (id-group) in parentheses like `:ID(order-reference)`.
...
* `:START_ID`, `:END_ID` - relationship file columns referring to the node ids. For id-groups, use `:END_ID(order-reference)`.
...
There is also a lesson in the link:https://neo4j.com/graphacademy/online-training/introduction-to-neo4j-40/[Introduction to Neo4j 4.0_ online course] that covers using the neo4j-admin import tool.
